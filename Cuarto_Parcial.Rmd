...
author: "Sofía Palacios Cuevas, Ana Paola Arias Martínez, Alejandro Gutierrez Figueroa"
date: "06/11/2021"
...
tics.sex<- rep(c("m CPU", "m laptop", "m tablet", "m smatphone", "m TV digital","h CPU", "h laptop", "h tablet", "h smartphone", "h TV digital"  ),2)
...
tics.niv<- rep(c("CPU", "laptop", "tablet", "smartphone", "TV digital"  ),2)
...
<div align="justify"> En esta gráfica se muestra de manera cualitativa la cantidad de jóvenes que estudian vs jóvenes que no estudian. Podemos notar que en ambos casos (hombres y mujeres) hubo una disminución en los casos de estudiantes.</div>
...
<div align="justify"> A continuación, realizaremos la prueba de rangos de signo de Wilcoxon. Este es un método no paramétrico de análisis de datos que usaremos para estudiar la distribución de la deserción escolar y del uso de TICs en dos momentos diferentes de tiempo (2019/2020 y 2020/2021). Para ello, utilizaremos la librería <code>coin</code>:</div>

```{r}
## Para la deserción escolar por sexo en el ciclo 2019/2020 vs la del ciclo 2020/2021
library(coin)
## Como "actual" y "pasado" son chr, debemos convertirlas a factor y luego a numéricas
ciclo.sex.fac<-as.factor(ciclo.sex)
ciclo.sex.num<-as.numeric(ciclo.sex.fac)
wilcox.test(x = value.sex, y = ciclo.sex.num, alternative = "two.sided", mu = 0, paired = TRUE)
```
```{r}
## Para el uso de TICs (gral) en el ciclo 2019/2020 vs en el ciclo 2020/2021
tics.ciclo.sex.fac<-as.factor(tics.ciclo.sex)
tics.ciclo.sex.num<-as.numeric(tics.ciclo.sex.fac)
wilcox.test(x = value.tics.sex, y = tics.ciclo.sex.num, alternative = "two.sided", mu = 0, paired = TRUE)
```

<div align="justify"> Ahora lo hacemos por cada uno de los 7 niveles educativos registrados en el estado de Querétaro.</div>
```{r}
# Para el uso de TICs (por nivel) en el ciclo 2019/2020 vs en el ciclo 2020/2021
tics.ciclo.niv.fac<-as.factor(tics.ciclo.niv)
tics.ciclo.niv.num<-as.numeric(tics.ciclo.niv.fac)

# Preescolar (NIV 1) 
wilcox.test(x = value.tics.niv1, y = tics.ciclo.niv.num, alternative = "two.sided", mu = 0, paired = TRUE)
```

```{r}
# Primaria (NIV 2) 
wilcox.test(x = value.tics.niv2, y = tics.ciclo.niv.num, alternative = "two.sided", mu = 0, paired = TRUE)
```

```{r}
# Secundaria (NIV 3)
wilcox.test(x = value.tics.niv3, y = tics.ciclo.niv.num, alternative = "two.sided", mu = 0, paired = TRUE)
```

```{r}
# CT sec. terminada (NIV 4)
wilcox.test(x = value.tics.niv4, y = tics.ciclo.niv.num, alternative = "two.sided", mu = 0, paired = TRUE)
```

```{r}
# Preparatoria (NIV 5)
wilcox.test(x = value.tics.niv5, y = tics.ciclo.niv.num, alternative = "two.sided", mu = 0, paired = TRUE)
```

```{r}
# CT prep. terminada (NIV 6)
wilcox.test(x = value.tics.niv6, y = tics.ciclo.niv.num, alternative = "two.sided", mu = 0, paired = TRUE)
```

```{r}
# Lic
wilcox.test(x = value.tics.niv7, y = tics.ciclo.niv.num, alternative = "two.sided", mu = 0, paired = TRUE)
```
